...
let charArray = [];
let numberOfExcerpts = 1;


function abschnitte() {
    let totalExcerpts = (Math.ceil(text.length / 100)); // AUSGABE 27

    for (let i = 0; i < text.length; i += 100) { // von 0, 100, 200, 300 ,...
        const sliceText = text.slice(0 + i, 99 + i); // Schneidet den Text dynamisch angefangen bei 0-99, 100-199, ...
        charArray.push(`${sliceText} - ${numberOfExcerpts++} of ${totalExcerpts}`); // Textausschnitte werden in ein gepush +
        // hinten wird - x of 27 drangehangen
    }
